...
public class MakeAppointmentController {
    ...
    public ResponseEntity<Object> makeAppointment(@RequestBody MakeAppointmentRequestDTO request) {
        logger.info("Received request to schedule an appointment: {}", request);

        // Validate that the start time is before the end time
        if (request.getAppointmentWindowStart().isAfter(request.getAppointmentWindowEnd())) {
            logger.warn("Appointment creation failed: Start time {} is after end time {}.",
                    request.getAppointmentWindowStart(), request.getAppointmentWindowEnd());
        ...
        if (request.getLicensePlate() == null || request.getLicensePlate().isEmpty()) {
            logger.warn("Invalid license plate: {}", request.getLicensePlate());
            return ResponseEntity.badRequest().body("License plate cannot be empty.");
        }

        if (MaterialType.fromString(request.getMaterialType()) == null) {
            logger.warn("Invalid material type: {}", request.getMaterialType());
        ...
        MakeAppointmentCommand command = new MakeAppointmentCommand(
                ...
                MaterialType.fromString(request.getMaterialType()),
                request.getAppointmentWindowStart(),
                request.getAppointmentWindowEnd(),
                new SellerId(request.getSellerId()) // Using sellerId directly as a UUID
        );
